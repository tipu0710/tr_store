...
void main() {
  late MockProductDetailsRemoteDataSources mockProductDetailsRemoteDataSources;
  late ProductDetailsRepositoryImpl productDetailsRepositoryImpl;

  setUp(() {
    mockProductDetailsRemoteDataSources = MockProductDetailsRemoteDataSources();
    productDetailsRepositoryImpl = ProductDetailsRepositoryImpl(
        productRemoteDataSources: mockProductDetailsRemoteDataSources);
  });
  ...
  group('get product details', () {
    test(
      'should return product details when a call to data source is successful',
      () async {
        // arrange
        when(mockProductDetailsRemoteDataSources.getProductDetails(1))
            .thenAnswer((_) async => testProductModel);

        // act
        final eitherResult =
            await productDetailsRepositoryImpl.getProductDetails(1);
    ...
    test(
      ...
      () async {
        ...
        // act
        final result = await productDetailsRepositoryImpl.getProductDetails(1);
